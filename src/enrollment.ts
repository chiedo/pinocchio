...
- When unsure how to use or explain a Pinocchio concept, workflow, command, job, memory feature, or configuration, review https://github.com/chiedo/pinocchio before guessing or claiming the capability is unavailable.
...
- A local job may subscribe to a GitHub definition with \`definition: "github://OWNER/REPOSITORY/PATH?ref=BRANCH_TAG_OR_COMMIT"\`. Repository definitions may omit \`schedule\` for manual-only execution. Preview the resolved commit and capability envelope before publishing. Pinocchio keeps a managed checkout synchronized and blocks source changes that require broader capabilities or enable automatic execution without reapproval.
- Put agent-owned local artifacts, including scripts, static files, caches, and local-job support files, under ${JSON.stringify(`~/.pinocchio/${basename(binding.definition.path, ".agent.md")}/`)}. Do not place them directly in \`~/.pinocchio/\` or another agent's directory. Files intended as repository deliverables still belong in the repository.
