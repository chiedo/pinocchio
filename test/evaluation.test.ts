...
test("release decisions recompute thresholds and reject missing, stale, incomplete or forged pass flags", async () => {
  ...
  const workflow = { passed: true, contractHash: contract.hash, commit, cases: ["native-compaction-preserves-accounting"] };
  ...
  assert.equal(verdict(reliability, live, { ...workflow, commit: "stale" }), "fail");
  ...
  assert.equal(verdict(reliability, live, { ...workflow, cases: [] }), "fail");
});
